...
> **2026-09-22 시점의 스냅샷이다. 이후 결정이 이 문서를 덮어썼다 — 현재 상태는 `CLAUDE.md`를 본다.**
> 특히 아래 본문이 처방하는 두 가지는 지금 **금지**다: `pumpAndSettle`(영구 애니메이션에서 10분 매달림,
> 경계 있는 `settle`로 교체됨)과 `--dart-define` 자격증명 주입(설계 원칙 4 — 자격증명은 요구하지 않는다).
> 문서 말미의 "사용자 결정 대기" 3건도 이미 결정됐다.
